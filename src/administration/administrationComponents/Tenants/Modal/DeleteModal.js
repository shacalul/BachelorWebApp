...
const DeleteModal = ({ id, disabled, onDeleteComplet }) => {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const roomBookings = await getRoomBookings();
      const userRoomBookings = roomBookings.filter(
        (booking) => booking.userId === id
      );
      const deletePromises = userRoomBookings.map((booking) =>
        deleteRoomBookings(booking.id)
      );
